...
export function formatLogMetrics(item, formatTokenCount, includeModel = true) {
  ...
  const totalTokens = finiteMetric(item.total_tokens)
    ?? (promptTokens !== null && completionTokens !== null
      ? promptTokens + completionTokens
      : null)
  if (promptTokens !== null || completionTokens !== null) {
    const promptTokenLabel = promptTokens === null ? '—' : formatTokenCount(promptTokens)
    const completionTokenLabel = completionTokens === null
      ? '—'
      : formatTokenCount(completionTokens)
    parts.push(`输入 ${promptTokenLabel} · 输出 ${completionTokenLabel}`)
...
function summarizeUsage(
  ...
  const hasTokenUsage = promptTokens || completionTokens
  const hasEstimatedCost = estimatedCost !== null && estimatedCost !== undefined
  const label = [
    callCount && `AI ${callCount} 次`,
    hasTokenUsage && `输入 ${formatTokenCount(promptTokens || 0)}`,
    hasTokenUsage && `输出 ${formatTokenCount(completionTokens || 0)}`,
    hasEstimatedCost && `¥${formatLogCost(estimatedCost)}`,
  ].filter(Boolean).join(' · ')
  const detail = [
    callCount && `${callCount} 次 AI 调用`,
    `输入 ${formatTokenCount(promptTokens || 0)}`,
    `输出 ${formatTokenCount(completionTokens || 0)}`,
    hasEstimatedCost && `本机估算 ¥${formatLogCost(estimatedCost)}`,
    unreportedCount && `${unreportedCount} 次未返回用量`,
  ].filter(Boolean).join(' · ')
  return { label, detail }
}
...
export function usageForTask(task, logs, formatTokenCount) {
  ...
  const reportedCalls = taskCalls.filter((call) => (
    finiteMetric(call?.prompt_tokens) !== null
    || finiteMetric(call?.completion_tokens) !== null
  ))
  if (taskCalls.length) {
    return summarizeUsage({
      callCount: taskCalls.length,
      promptTokens: reportedCalls.reduce(
        (sum, call) => sum + (finiteMetric(call.prompt_tokens) || 0),
        0,
      ),
      completionTokens: reportedCalls.reduce(
        (sum, call) => sum + (finiteMetric(call.completion_tokens) || 0),
        0,
      ),
      estimatedCost: taskCalls.reduce(
        (sum, call) => sum + (finiteMetric(call.estimated_cost) || 0),
        0,
      ),
      unreportedCount: taskCalls.length - reportedCalls.length,
    }, formatTokenCount)
  }
  const metricLogs = logs.filter((item) => (
