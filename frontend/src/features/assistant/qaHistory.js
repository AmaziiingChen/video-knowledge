...
export function savedQaHistoryItems(items) {
  ...
  return items
    ...
    .map((item) => ({
      ...
      answer: item.answer,
      reasoning: typeof item.reasoning_content === 'string' ? item.reasoning_content : '',
      reasoningExpanded: false,
      suggestedQuestions: Array.isArray(item.suggested_questions) ? item.suggested_questions.slice(0, 3) : [],
      saved: true,
      pending: false,
      error: false,
