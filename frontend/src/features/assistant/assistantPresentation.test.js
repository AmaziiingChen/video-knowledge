...
test('retains disabled metadata for the selected provider model', () => {
  ...
  assert.equal(options[0].provider, 'qwen')
  assert.equal(options[0].providerLabel, '阿里云百炼 · 千问')
  assert.equal(assistantSelectedModelLabel(options[0].value, options), 'Qwen Plus')
})
