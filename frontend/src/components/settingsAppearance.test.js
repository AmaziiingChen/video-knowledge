...
const telemetrySettingsControllerSource = await readFile(new URL('../features/telemetry/useTelemetrySettingsController.js', import.meta.url), 'utf8')
...
test('privacy settings disclose Cloudflare delivery and the three-month retention boundary', () => {
  ...
  assert.match(source, /新安装默认开启/)
  assert.match(source, /低频发送 17 个固定检查点到 Cloudflare/)
  assert.match(source, /数据最多保留 3 个月/)
  assert.match(source, /只保留“不发送”的本机偏好/)
  assert.match(source, /:disabled="!telemetryStatusLoaded \|\| telemetryStatusLoading \|\| telemetrySaving"/)
  assert.match(source, /正在读取当前诊断状态/)
  assert.match(telemetrySettingsControllerSource, /无法读取当前诊断状态/)
  assert.match(source, /@click="loadTelemetryStatus">重试/)
  assert.match(source, /useTelemetrySettingsController/)
