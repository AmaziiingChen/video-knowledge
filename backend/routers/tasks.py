...
from services.pipeline_runner import AICallInfo, DownloadTransferInfo, PipelineErrorInfo, PipelineLog, PipelineRequest, TextSourceInfo
...
class TaskResponse(BaseModel):
    ...
    # Keep the submitted link available before parsing finishes.  Queued tasks
    # otherwise have no platform in their PipelineResponse, which made the
    # desktop status bar fall back to the previous task's source.
...
def _to_response(record: TaskRecord, *, include_heavy_payload: bool = True) -> TaskResponse:
    ...
    return TaskResponse(
        ...
        # Queue polling only needs status and transfer metadata.  Returning
        # every historical transcript and summary repeatedly makes one slow
        ...
        # Persistence failures are separate from pipeline failures: keep the
        # successful result/status intact, while also exposing the local-save
        # problem through the existing diagnostic field used by clients.
...
async def create_task(req: PipelineRequest):
    return _to_response(task_manager.create(req))
...
async def list_tasks(
    ...
    return [_to_response(record, include_heavy_payload=False) for record in records]
