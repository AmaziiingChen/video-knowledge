...
from services.ai_call_logger import AICallRecord, tracked_llm_provider
...
def _normalize_citation_tokens(markdown: str, valid_ids: set[str]) -> str:
    """Repair the common ``[S001]`` typo before coverage is evaluated.

    The contract only accepts Markdown footnotes.  A model occasionally drops
    the caret after a valid footnote (``[^S001][S014]``); leaving it literal
    makes the second source invisible to the audit and to the publisher.
    Restrict conversion to the source IDs assigned to this report so ordinary
    bracketed text and unknown IDs are left untouched for the normal validator.
    """
...
def _coalesce_repeated_single_source_citations(markdown: str) -> str:
    ...
    The model still decides which source supports a statement. This only removes
    visually noisy repeats when every citation in one uninterrupted block is the
    same source. Mixed-source blocks are deliberately left untouched rather
    than weakening their provenance.
    """
    chunks = re.split(r"(\n{2,})", str(markdown or ""))
    ...
    for index in range(0, len(chunks), 2):
        ...
        if len(citations) < 2 or len(unique_ids) != 1:
            continue
        # Tables have their own cell-level semantics; retain their original
        # placements instead of guessing whether the citation belongs to a row.
...
def _normalize_generated_section_markdown(markdown: str) -> str:
    """Enforce deterministic Markdown and citation contracts on section output.

    The writer still decides facts, grouping and provenance. This pass only
    repairs presentation states that are always invalid in a generated section:
    model-authored footnote definitions, detached citation-only lines, citation
    bundles placed outside the final table cell, and orphaned continuation rows.
    """
...
def _restore_interrupted_markdown_tables(lines: list[str]) -> list[str]:
    ...
    while index < len(lines):
        ...
        if (
            ...
            and _is_markdown_table_separator(next_line)
        ):
...
def _normalize_section_heading_levels(markdown: str, section_title: str) -> str:
    ...
    for line in str(markdown or "").splitlines():
        ...
        if not normalized and " ".join(heading_text.split()) == normalized_section_title:
            # The system has already rendered this H2; do not let an obedient
            # but redundant model response create a second visual title.
            continue
        requested_level = len(match.group(1))
        # The enclosing section is H2. Model-authored headings begin at H3;
        # a child may only descend one level at a time.
...
def _normalize_report_overview(markdown: str) -> str:
    ...
    for line in str(markdown or "").strip().splitlines():
        ...
        if heading_text in {"报告正文", "本期概览"}:
            continue
        # The overview has exactly one system-owned heading. Keep an
        # unexpected model subheading as readable text rather than allowing
        # it to introduce another Markdown level into the report outline.
    ...
    if (
        len(blocks) >= 3
        and not any(re.match(r"^[-*+]\s+", block) for block in blocks)
    ):
        body = blocks[0] + "\n\n" + "\n".join(
            f"- {block}" for block in blocks[1:]
        )
    return "## 本期概览" + (f"\n\n{body}" if body else "")
...
def _append_footnotes(markdown: str, sources: list[GroupReportSource], cited_ids: set[str]) -> str:
    references = []
    for source in sources:
        if source.citation_id not in cited_ids:
            continue
        references.append(_source_footnote(source))
...
def _source_footnote(source: GroupReportSource) -> str:
    ...
    # Keep the link label as the title and put provenance after it.  The
    # renderer uses this canonical order for both inline citation cards
    # and the source list, while retaining the original article URL.
