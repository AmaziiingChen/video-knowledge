...
from services.wechat_subscription_secrets import (
    SessionCredentials,  # noqa: F401 - legacy public import
...
class WeChatSubscriptionService:
    ...
    def connect_account(self, *, display_name: str, token: str, cookie: str) -> dict[str, Any]:
        return self._accounts.connect_account(display_name=display_name, token=token, cookie=cookie)

    def complete_qr_login(self, status: QrLoginStatus, display_name: str = "") -> dict[str, Any]:
        return self._accounts.complete_qr_login(status, display_name)

    def reauthorize_account(self, account_id: str, *, token: str, cookie: str) -> dict[str, Any]:
        return self._accounts.reauthorize_account(account_id, token=token, cookie=cookie)

    def complete_qr_reauthorization(self, account_id: str, status: QrLoginStatus) -> dict[str, Any]:
        return self._accounts.complete_qr_reauthorization(account_id, status)

    def list_accounts(self) -> list[dict[str, Any]]:
        return self._accounts.list_accounts()

    def get_account(self, account_id: str) -> dict[str, Any]:
        return self._accounts.get_account(account_id)
    ...
    def transfer_subscriptions(self, source_account_id: str, target_account_id: str) -> dict[str, Any]:
        return self._accounts.transfer_subscriptions(source_account_id, target_account_id)

    def search_accounts(self, account_id: str, query: str, limit: int = 10) -> list[WeChatAccountCandidate]:
        if not query.strip():
            return []
        with self._remote_sync_lock:
            credentials = self._accounts.credentials_for_request(account_id)
            try:
                self._accounts.consume_request_budget(account_id)
                results = self._admin_client.search_accounts(credentials, query, limit=limit)
            except WeChatRateLimitError as exc:
                self._accounts.mark_rate_limited(account_id, exc)
                raise
            except WeChatAuthorizationError:
                self._accounts.mark_account_reauth(account_id)
                raise
            self._accounts.clear_rate_limit(account_id)
        return results
        ...
        with self._remote_sync_lock:
            ...
            try:
                articles = self._admin_client.list_articles(
                    ...
                    before_request=lambda: self._accounts.consume_request_budget(account_id),
                    **list_options,
                )
            except WeChatRateLimitError as exc:
                self._accounts.mark_rate_limited(account_id, exc)
