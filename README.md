...
本项目的 MCP 服务也提供公众号搜索、订阅、列出订阅、读取最新文章和导出单篇文章 Markdown 工具；它们只使用本机 API，微信凭据仍保存在 macOS Keychain。安装 OpenClaw 后，可在 KnowledgeHub 的“设置 → 微信链接自动处理”明确点击“修复 MCP”，只更新 `knowledgehub` 这一条本机命令配置并自检；它仅在 KnowledgeHub 正在运行、短期 bridge lease 有效时可用。详细边界见 [OpenClaw 接入说明](docs/openclaw-ingest.md#local-mcp-bridge-configuration)。
